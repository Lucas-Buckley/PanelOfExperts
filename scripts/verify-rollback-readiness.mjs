#!/usr/bin/env node
/**
 * Purpose: Verifies rollback readiness prerequisites for release and incident response.
 * Inputs: Repository files, optional env/CLI values, and optional remote URL probes.
 * Outputs: Exit code 0 when readiness checks pass, non-zero with actionable errors on failure.
 */
...
function assertCondition(condition, message, errors) {
  /**
   * Purpose: Collects rollback readiness failures into a shared error list.
   * Inputs: Boolean condition, failure message, and mutable error array.
...
function readJsonFile(filePath) {
   ...
   * Inputs: Absolute file path to JSON file.
...
function parseBoolean(raw, fallback) {
  /**
   * Purpose: Parses boolean-like strings with fallback behavior.
   * Inputs: Raw string-like value and fallback boolean.
   * Outputs: Parsed boolean value or fallback when invalid/missing.
   */
  if (raw === undefined) {
    return fallback;
  }
...
function parseEnvExampleKeys(rawEnvFile) {
  /**
   * Purpose: Extracts environment variable keys declared in `.env.example`.
   * Inputs: Raw `.env.example` file text.
   * Outputs: Set of discovered environment variable names.
   */
  const keys = new Set();
...
function collectMigrationSqlFiles(migrationsDir) {
  /**
   * Purpose: Collects migration SQL file paths from Prisma migrations directory.
   * Inputs: Absolute path to `prisma/migrations`.
   * Outputs: Array of migration SQL file paths.
   */
...
function runPrismaValidate(shouldSkip, requireDatabaseEnv, errors) {
  /**
   * Purpose: Runs Prisma schema validation as part of rollback readiness checks.
   * Inputs: Skip flag, strict-env flag, and mutable error array.
   * Outputs: None; appends failure details when validation fails.
   */
  if (shouldSkip) {
  ...
  if (!hasDatabaseUrl || !hasDirectUrl) {
    if (requireDatabaseEnv) {
      errors.push(
        "Prisma validate requires DATABASE_URL and DIRECT_URL when --require-prisma is enabled."
      );
      return;
    }
  ...
  errors.push(
    `Prisma validate failed with exit code ${result.status}. ${stderr || stdout || "No output."}`
  );
}

async function runRemoteProbes(baseUrl, bypassToken, timeoutMs, errors) {
  /**
   * Purpose: Runs remote HTTP checks useful for post-rollback verification.
   * Inputs: Base URL, optional bypass token, timeout milliseconds, and mutable error list.
   * Outputs: None; appends probe failures when expectations are not met.
   */
  if (!baseUrl) {
  ...
  try {
    ...
    assertCondition(
      ...
      "Rollback probe GET /api/panels must return 401 when unauthenticated.",
      errors
    );
...
async function main() {
  /**
   * Purpose: Executes rollback readiness checks and reports pass/fail output.
   * Inputs: Process args/env values.
   * Outputs: Console report and process exit code.
   */
  const errors = [];
  ...
  const migrateWorkflowPath = path.resolve(
    rootDir,
  ...
  assertCondition(
    fs.existsSync(migrateWorkflowPath),
    "Missing prisma-migrate-deploy workflow.",
    errors
  );
  ...
  if (errors.length === 0) {
    ...
    assertCondition(
      ...
      "No Prisma migration.sql files found under prisma/migrations.",
      errors
    );
  }

  const skipPrisma =
    parseBoolean(args["skip-prisma"], false) ||
    parseBoolean(process.env.ROLLBACK_SKIP_PRISMA, false);
  const requirePrisma =
    parseBoolean(args["require-prisma"], false) ||
  ...
  const timeoutMs = parsePositiveInt(
    args["timeout-ms"] ?? process.env.ROLLBACK_TIMEOUT_MS,
    20000
  );
...
main().catch((error) => {
  console.error(
    `Rollback readiness verification failed: ${error instanceof Error ? error.message : String(error)}`
  );
  process.exit(1);
});
