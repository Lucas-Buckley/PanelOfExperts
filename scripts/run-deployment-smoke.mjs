...
async function runSmoke() {
    ...
    const bypassToken = cliArgs["bypass-token"] ?? process.env.VERCEL_BYPASS_TOKEN ?? "";
    const timeoutMs = parsePositiveInt(cliArgs["timeout-ms"] ?? process.env.SMOKE_TIMEOUT_MS, 60000);
    const expectedMode = normalizeExpectedMode(cliArgs["expect-mode"] ?? process.env.SMOKE_EXPECT_MODE);
    const expertCount = parsePositiveInt(cliArgs.experts ?? process.env.SMOKE_EXPERT_COUNT, expectedMode === "live" ? 1 : 2);
    ...
    assertCondition(register.status === 201, `register failed: status=${register.status} body=${JSON.stringify(register.data)}`);
    ...
    assertCondition(panel.status === 201, `create panel failed: status=${panel.status} body=${JSON.stringify(panel.data)}`);
    ...
    assertCondition(conversation.status === 201, `create conversation failed: status=${conversation.status} body=${JSON.stringify(conversation.data)}`);
    const conversationId = conversation.data?.id;
    assertCondition(Number.isInteger(conversationId) && conversationId > 0, "create conversation missing valid id");
    ...
    assertCondition(firstPrompt.status === 201, `first prompt failed: status=${firstPrompt.status} body=${JSON.stringify(firstPrompt.data)}`);
    assertCondition(Array.isArray(firstPrompt.data?.responses) &&
        firstPrompt.data.responses.length === expertCount, `first prompt responses mismatch: expected=${expertCount} got=${firstPrompt.data?.responses?.length ?? 0}`);
    ...
    assertCondition(secondPrompt.status === 201, `second prompt failed: status=${secondPrompt.status} body=${JSON.stringify(secondPrompt.data)}`);
    assertCondition(Array.isArray(secondPrompt.data?.responses) &&
        secondPrompt.data.responses.length === expertCount, `second prompt responses mismatch: expected=${expertCount} got=${secondPrompt.data?.responses?.length ?? 0}`);
    console.log("5) second prompt: ok");
    const reopenedConversation = await request(`/api/conversations/${conversationId}`, {
        token: accessToken
    });
    assertCondition(reopenedConversation.status === 200, `open conversation failed: status=${reopenedConversation.status} body=${JSON.stringify(reopenedConversation.data)}`);
    assertCondition(Array.isArray(reopenedConversation.data?.prompts) &&
        reopenedConversation.data.prompts.length === 2, "conversation persistence check failed");
    ...
    const simulatedMarkers = allResponses.filter((content) => content.includes("[SIMULATED:")).length;
    ...
    if (expectedMode === "live") {
        ...
        assertCondition(simulatedMarkers === allResponses.length, "expected simulated markers in all responses while expecting simulated mode");
    }
