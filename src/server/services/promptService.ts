...
import { estimatePanelMaxTokens, runPanel, type PanelRunnerHistoryPromptInput } from "./panelRunner";
...
class PromptServiceError extends Error {
    ...
    status: 400 | 404 | 429;
    constructor(code: "VALIDATION" | "NOT_FOUND" | "RATE_LIMIT", status: 400 | 404 | 429, message: string) {
        super(message);
        this.code = code;
        this.status = status;
    }
...
async function reserveDailyTokens(estimatedTokens: number): Promise<LlmDailyUsageReservation> {
    ...
    if (maxUsedBeforeReservation < 0) {
        throw new PromptServiceError("RATE_LIMIT", 429, `Estimated request size exceeds daily cap of ${appConfig.llmMaxDailyTokens.toLocaleString()} tokens.`);
    }
    ...
    if (reservationResult.count !== 1) {
        throw new PromptServiceError("RATE_LIMIT", 429, `Daily token cap reached (${appConfig.llmMaxDailyTokens.toLocaleString()} tokens/day).`);
    }
    return {
        usageDate,
        model,
...
async function reconcileReservedDailyTokens(reservation: LlmDailyUsageReservation, actualTokens: number): Promise<void> {
...
function mapPromptHistoryForRunner(rows: Array<{
    sequence: number;
    content: string;
    responses: Array<{
        expertId: number;
    ...
    const chronologicalRows = [...rows].sort((left, right) => left.sequence - right.sequence);
...
function estimateHistoryPromptSize(prompt: PanelRunnerHistoryPromptInput): number {
    const responseSize = prompt.responses.reduce((total, response) => total +
        response.content.length +
        response.expertName.length +
        response.sequence.toString().length, 0);
    return prompt.content.length + prompt.sequence.toString().length + responseSize;
}
function applyHistoryBudget(history: PanelRunnerHistoryPromptInput[], charBudget: number): PanelRunnerHistoryPromptInput[] {
    if (history.length === 0) {
        return [];
    }
    let used = 0;
    ...
    const includesFirstPrompt = selectedRecent.some((item) => item.sequence === firstPrompt.sequence);
...
export async function createPromptForConversation(accountId: number, conversationId: number, input: unknown): Promise<PromptWithResponsesView> {
...
export function mapPromptErrorToHttp(error: unknown): {
    status: number;
    message: string;
} {
    ...
    if (error instanceof Prisma.PrismaClientKnownRequestError &&
        ...
        return {
            status: 503,
            message: "Database schema is out of date for prompt processing. Run migrations with `npm run db:migrate:deploy` and retry."
        };
    }
    return {
        status: 500,
