...
type GenerateResponseMock = (input: {
    prompt: string;
    model?: string;
}) => Promise<{
    request_id: string;
...
async function importPanelRunnerWithMock(mode: "simulated" | "live", generateResponseMock: GenerateResponseMock) {
    vi.resetModules();
    process.env = {
        ...originalEnv,
...
describe("panel runner", () => {
    it("first-turn first expert prompt excludes prior context section", async () => {
        ...
        expect(seenPrompts[0]).toContain("Write naturally and avoid labeled sections, headings, or list formatting unless the user asks for it.");
        expect(seenPrompts[0]).toContain("Default to 2-4 short paragraphs unless the user asks for a different format.");
    ...
    it("later experts receive prior same-turn outputs", async () => {
        ...
        const { runPanel } = await importPanelRunnerWithMock("simulated", async ({ prompt }) => {
            seenPrompts.push(prompt);
            callCount += 1;
            return createMockLlmResponse(callCount === 1
                ? "First expert answer"
        ...
        expect(seenPrompts[1]).toContain("Prefer plain prose over bullets or outline formatting unless the user explicitly asked for a list.");
        expect(seenPrompts[1]).toContain("Default to 2-4 short paragraphs; avoid one oversized paragraph unless the user asked for that style.");
        ...
        expect(seenPrompts[1]).not.toContain("3) Caveat or disagreement:");
    ...
    it("retries downstream expert once when no prior-expert reference is present", async () => {
        ...
        expect(seenPrompts[2]).toContain("Your previous attempt sounded too templated or did not satisfy the response rules.");
        expect(seenPrompts[2]).toContain("Do not use headings, label-like transitions, or meta phrases such as 'from a ... lens', 'from my specialization', or 'a practical caveat'.");
    ...
    it("retries first expert once when response uses templated meta-framing", async () => {
        ...
        const { runPanel } = await importPanelRunnerWithMock("simulated", async ({ prompt }) => {
            ...
            callCount += 1;
            if (callCount === 1) {
                return createMockLlmResponse("From a moral-philosophical lens, the trolley problem asks whether intervention is justified.");
            }
            return createMockLlmResponse("The trolley problem asks whether intervening to save more lives can justify directly causing one death.");
        });
        const result = await runPanel({
            conversationId: 9,
            panelId: 3,
        ...
        expect(seenPrompts[1]).toContain("Regenerate the answer so it sounds natural and conversational.");
