...
describe("llm provider", () => {
    ...
    it("rejects model overrides that violate pinning", async () => {
        ...
        process.env.LLM_ENABLED = "true";
        const { generateResponse } = await importFreshLlmModule();
        await expect(generateResponse({ prompt: "hello", model: "gpt-4.1-mini" })).rejects.toThrow("Model must be pinned");
    });
    it("fails fast when llm is disabled", async () => {
        ...
        const { generateResponse } = await importFreshLlmModule();
        await expect(generateResponse({ prompt: "hello" })).rejects.toThrow("LLM is disabled");
    });
    ...
    it("retries live generation once when no text is returned due to max_output_tokens", async () => {
        ...
        expect(createMock.mock.calls[1]?.[0].input).toContain("Output length requirement for this retry:");
