...
import { createConversationForAccount, deleteConversationForAccount, getConversationForAccount, listConversationsForPanelForAccount, renameConversationForAccount } from "../../src/server/services/conversationService";
beforeEach(() => {
...
describe("conversation service", () => {
    it("conversation creation requires valid panel ownership", async () => {
        ...
        await expect(createConversationForAccount(5, {
            panelId: 999,
            name: "Planning Chat"
    ...
    it("conversation retrieval returns prompts/responses in stable order", async () => {
        ...
        expect(prismaMock.conversation.findFirst).toHaveBeenCalledWith(expect.objectContaining({
            where: {
                id: 77,
                panel: {
                    accountId: 5
    ...
    it("denies conversation rename for non-owner", async () => {
        prismaMock.conversation.findFirst.mockResolvedValue(null);
        await expect(renameConversationForAccount(5, 77, {
            name: "Renamed Conversation"
        })).rejects.toMatchObject({
            status: 404
        });
