...
describe("prompt service", () => {
    it("rejects invalid conversation ownership", async () => {
        ...
        await expect(createPromptForConversation(7, 88, {
            content: "hello"
    ...
    it("creates prompt before responses and links response foreign keys correctly", async () => {
        ...
        tx.prompt.create.mockImplementation(async (args: {
            data: {
                sequence: number;
            };
        }) => {
        ...
        tx.response.create.mockImplementation(async (args: {
            data: {
                expertId: number;
                promptId: number;
            };
        ...
        expect(panelRunnerMock.runPanel).toHaveBeenCalledWith(expect.objectContaining({
            panelName: "Launch Council"
        }));
        expect(tx.response.create).toHaveBeenNthCalledWith(1, expect.objectContaining({
            data: expect.objectContaining({
                promptId: 501,
                expertId: 2
            })
        }));
        expect(tx.response.create).toHaveBeenNthCalledWith(2, expect.objectContaining({
            data: expect.objectContaining({
                promptId: 501,
                expertId: 5
            })
        }));
        expect(tx.prompt.create).toHaveBeenCalledWith(expect.objectContaining({
            data: expect.objectContaining({
                llmInputTokens: 80,
                llmOutputTokens: 120,
                llmTotalTokens: 200
