...
import { createIdempotencyRequestHash, executeWithIdempotency, mapIdempotencyErrorToHttp, readIdempotencyKeyFromRequest } from "../../src/server/http/idempotency";
...
describe("idempotency http helper", () => {
    ...
    it("rejects reused key when payload hash differs", async () => {
        ...
        await expect(executeWithIdempotency({
            accountId: 7,
            endpoint: "/api/conversations/[conversationId]/prompts",
            method: "POST",
            idempotencyKey: "prompt:xyz",
